...
int main() {
    ...
    for (int i = 0; i < simulations; i++) {
        ...
        mt19937 gen_local(rd() + i);
        normal_distribution<> norm_local(0.0, 1.0);
        for (int d = 0; d < days; d++) {
            double z = norm_local(gen_local);
            double dt = 1.0;
    ...
    cout << "GBM+Monte Carlo Prediction with OpenMP+MPI(" << days << " days)" << endl;
